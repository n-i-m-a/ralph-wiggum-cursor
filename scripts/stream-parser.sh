...
# Thresholds (allow env overrides for reliability tuning)
  ...
  case "$type" in
    ...
    "tool_call")
      if [[ "$subtype" == "started" ]]; then
        ...
        # Handle write tool completion
          ...
          local lines
          lines=$(echo "$line" | jq -r '.tool_call.writeToolCall.result.success.linesCreated // 0' 2>/dev/null) || lines=0
          local bytes
    ...
    "result")
      ...
      if [[ "$TOOL_CALLS" -eq 0 ]]; then
        log_error "⚠️ NO_ACTIVITY: session ended with zero tool calls"
        echo "NO_ACTIVITY" 2>/dev/null || true
      fi
      ;;
  esac
}
